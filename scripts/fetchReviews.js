/**
 * Google Reviews Fetcher Script - Versión ESM
 *
 * Este script obtiene reviews de Google Maps usando la API de Outscraper (free tier).
 *
 * INSTRUCCIONES:
 * - Ya configurado con tu API key y URL de Google Maps
 * - Ejecutar: node scripts/fetchReviews.js
 * - Descarga reviews y avatares automáticamente
 * - Actualiza src/data/googleReviews.json
 *
...
// ============================================
// Obtener __dirname en ESM (Node.js 18+)
// ============================================

const __filename = fileURLToPath(import.meta.url);
const __dirname = path.dirname(__filename);

// fileURLToPath maneja correctamente la decodificación de espacios y rutas Windows
const fixedDirname = __dirname;
...
const CONFIG = {
  // API key de Outscraper (100 requests/mes gratis)
  OUTSCRAPER_API_KEY: 'c2MjNjVjNTAzOGNiOTExNDIzMmE3MWNkN2E2NzBlMzIxZjV8NmM0NWM0ZjIzYg',

  // URL de Google Maps del negocio del Dr. Aldo Avellaneda
  GOOGLE_MAPS_URL: 'https://www.google.com/maps/place/Ortopedista,+Traumatología+Mérida,+Cancún+%7C+Dr.+Aldo+Avellaneda+%7C+Especialista+en+Columna+%7C+Cirugía+Mínima+Invasión/@21.0161907,-89.5850012,17z/data=!4m6!3m5!1s0x8f5676f669555555:0xd6a48100ff0e2fe7!8m2!3d21.0161907!4d-89.5850012!16s%2Fg%2F11kjh881jg?hl=es&entry=ttu&g_ep=EgoyMDI1MTIwOS4wIKXMDSoASAFQAw%3D%3D',

  // Máximo de reviews a obtener (recomendado: 15-20)
  MAX_REVIEWS: 20,

  // Idioma de las reviews (es = español)
  LANGUAGE: 'es',

  // Rutas de salida
  OUTPUT_PATH: path.join(fixedDirname, '../src/data/googleReviews.json'),
  AVATARS_DIR: path.join(fixedDirname, '../public/reviews/avatars'),
};
...
async function fetchGoogleReviews() {
  ...
  try {
    ...
    try {
      ...
      console.warn(`⚠️  No se pudo obtener datos de la API: ${apiError.message}`);
      console.log('🔄 Intentando usar datos locales de respaldo (src/data/outscraper-response.json)...');

      const fallbackPath = path.join(fixedDirname, '../src/data/outscraper-response.json');

      if (fs.existsSync(fallbackPath)) {
        try {
          const rawData = fs.readFileSync(fallbackPath, 'utf8');
          data = JSON.parse(rawData);
          console.log('✓ Datos cargados exitosamente desde el archivo local.\n');
        } catch (parseError) {
          throw new Error(`Error al leer/parsear el archivo local: ${parseError.message}`);
        }
      } else {
        throw new Error(`Falló la API y no se encontró el archivo local de respaldo en: ${fallbackPath}`);
      }
    }
